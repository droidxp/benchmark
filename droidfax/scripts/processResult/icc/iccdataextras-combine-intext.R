...
deicc<- matrix(NA, nrow=nrow(tdata), ncol=ncol(tdata)/4)
...
for(i in seq(1,nrow(tdata),1)) {
	...
	curdeicc<- c(f.per(tdata[i,1]+tdata[i,2]+tdata[i,3]+tdata[i,4],dicc), f.per(tdata[i,5]+tdata[i,6]+tdata[i,7]+tdata[i,8],dicc))
	deicc[r,] <- curdeicc
...
for(i in seq(1,nrow(tdatainter),1)) {
	...
	curdeinterICC<- c(f.per(tdatainter[i,1]+tdatainter[i,2]+tdatainter[i,3]+tdatainter[i,4],dinterICC), f.per(tdatainter[i,5]+tdatainter[i,6]+tdatainter[i,7]+tdatainter[i,8],dinterICC))
...
pdf("./deiccboth-intext.pdf",width=2.5,height=3.0)
deiccboth <- cbind ( deicc[,1],deinterICC[,1], deicc[,2],deinterICC[,2] )
boxplot(deiccboth, names=c("internal","","external",""),col=colors,ylab="percentage (instance view)",range=0,cex.axis=0.4,lwd=0.3,cex.lab=0.5)
