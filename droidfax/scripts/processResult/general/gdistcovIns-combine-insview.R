...
pdf("./gdist-ins-d-horiz.pdf",width=4.5,height=3.0)
...
if (horizontal) {
    boxplot(s, names=xnames,col=colors,xlab="percentage of calls", ylab="code layer", range=0,cex.axis=0.4, horizontal=TRUE, las=0, lwd=0.3,cex.lab=0.5)
    points(means, 1:6, col="red", pch=18, cex=0.5)
    legend("bottomright", legend=c("class", "method"), cex=.5, col=c("#ffff33","gray80"), lwd=4.5, bty="n")
} else {
    boxplot(s, names=xnames,col=colors,xlab="code layer", ylab="percentage of calls",range=0,cex.axis=0.4, lwd=0.3,cex.lab=0.5)
