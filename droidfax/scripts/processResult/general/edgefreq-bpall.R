...
pdf("./edgeFreq-bpall.pdf",width=4.0,height=0.5)
i=1
...
for (key in rev(allkeys)) {
	#print (key)
	vdata <- catdata[[key]]
	#print (vdata)xlim=c(1,700000), ylim=c(1,900000),
	#summary(vdata)
	if (i==1) {
		boxplot(vdata, range=0,cex.axis=0.4,lwd=0.3,cex.lab=0.5,log="x",horizontal=TRUE)
		...
		boxplot(vdata, range=0,cex.axis=0.4,lwd=0.3,cex.lab=0.5,horizontal=TRUE,log="x")
		mtext(side = 1, text = "Call", line = .5, cex=.5)
		mtext(side = 2, text = "Frequency", line = 1.0, cex=.5)
		meand<-c(meand,mean(vdata))
		points(meand, col="red", pch=18, cex=0.5)
